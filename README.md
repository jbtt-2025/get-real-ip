...
* ✍️ **防篡改签名 (Anti-Tampering)**: 支持对提取到的真实 IP 进行 HMAC-SHA256 签名，输出 `base64(IP).signature` 格式。下游服务可轻松验签，确保 IP 在内网传输过程中未被篡改。
...
2. 启动容器并挂载该文件：
   ...
   docker run -d \
     ...
     --restart always \
     -p 8080:8080 \
     -v $(pwd)/proxy_cidr.txt:/root/proxy_cidr.txt \
     -e SIGN_TOKEN="your_super_secret_token_here" \
     ghcr.io/jbtt-2025/get-real-ip:latest
   ```
...
```text
[Base64编码的真实IP].[HMAC-SHA256签名(Hex)]
```

**示例：**
`MTI3LjAuMC4x.7f83b1657ff1fc53b92dc18148a1d65dfc2d4b1fa3d67728bd5e3053abc400f1`
*(解码后为: `127.0.0.1`)*
...
4. 如果一致，说明 IP 未被篡改且确实来源于本服务，此时对 `Payload` 进行 Base64 解码，即可得到真正的客户端 IP；如果不一致，则丢弃该请求。
